...
public class DrawerUtil {
	...
	public static void RenderChildren(SerializedProperty p, Type componentType, int indentDelta = 1) {
		EditorGUI.indentLevel += indentDelta;
		string originName = p.name;
		while (p.NextVisible(true)) {
			if (p.name == originName) {//.StartsWith("__requirement")) {
				break;
			}
