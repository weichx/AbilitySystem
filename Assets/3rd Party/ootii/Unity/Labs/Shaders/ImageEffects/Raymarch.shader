...
float4 frag(posuv i) : COLOR
{
	...
	for (int i = 0; i < steps; i++)
	{
		...
		float3 shadowMapSample = tex2Dlod(_Shadowmap, float4(pos.xy, 0, 0)).x > pos.z;

		shadowMapSample *= attenuation(pos.z);

		#if defined(COLORED_ON)
			shadowMapSample *= tex2Dlod(_ColorFilter, float4(pos.xy, 0, 0)).xyz;
		#endif

		#if defined(SPOT_SHAFTS)
			shadowMapSample *= cookie(pos.xy);
		#endif

		inscatter += shadowMapSample;
	}
