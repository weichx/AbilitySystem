...
public class Serializer {
    ...
    public Serializer(params object[] inputs) {
        ...
        refId = 0;
        defaultSerializer = new TypeSerializer<object>();
    ...
    public void WriteDefault(object obj) {
       ...
       // PropertyInfo[] propertyInfoList = type.GetProperties(PropertyBindFlags);
        for (int i = 0; i < fieldInfoList.Length; i++) {
            FieldInfo fInfo = fieldInfoList[i];
            ...
            DoWrite(fieldType, val, obj);
        }

        //for (int i = 0; i < propertyInfoList.Length; i++) {
        //    PropertyInfo pInfo = propertyInfoList[i];
        //    Type propType = pInfo.PropertyType;
        //    object val = pInfo.GetValue(obj, null);

        //    body.Append(propType.Name);
        //    body.Append(" P ");
        //    body.Append(GetTypeName(propType));

        //    DoWrite(propType, val, obj);
        //}
    ...
    private void DoWrite(Type type, object val, object origin) {
        if (val == null) {
            ...
            GetSerializer(type).Serialize(val, this);
    ...
    private void Serialize(object obj) {
        ...
        GetSerializer(type).Serialize(obj, this);
